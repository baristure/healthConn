...
import { handleBodyPart } from "../appointmentSlice";
...
export const BodyComponent = ({ partName }) => {
  const dispatch = useDispatch();
  const { bodyParts } = useSelector((state) => state.appointment);

  const onClickSvg = ({ target }) => {
    dispatch(handleBodyPart(target));
  };
  ...
  return (
    <Wrapper className="human-body flex justify-center flex-col">
      {partName && svgElements[partName] && svgElements[partName](true)}
      {Object.keys(svgElements)
        .filter((part) => !bodyParts[part] || bodyParts[part].show !== false)
        .map((part) => {
          let selected = false;
          if (bodyParts[part]) selected = bodyParts[part].selected;
