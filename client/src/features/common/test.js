...
export default function Test() {
  ...
  return (
    <div className="min-h-screen bg-gray-200">
      <aside className="relative h-screen inline-flex flex-col justify-between items-center bg-white shadow p-6">
        <nav className="inline-flex flex-col space-y-2">
          {nav.map((item, index) => {
            return (
              <div
                className={`flex items-center text-gray-600 py-2 cursor-pointer ${
                  isEnlarge
              ...
              >
                ...
                {isEnlarge && (
                  <span className="font-medium select-none">item.text</span>
                )}
              </div>
            );
          })}
        ...
        <button
          onClick={() => setIsEnlarge((prev) => !prev)}
          className="absolute right-0 bottom-0 top-0 my-auto transform translate-x-full h-10 w-10 p-1 bg-white text-gray-600 rounded-r-lg mx-auto border border-solid border-gray-200 hover:border-gray-300"
        >
          {isEnlarge ? (
            <span>
              {
